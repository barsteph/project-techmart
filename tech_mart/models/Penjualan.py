...
class Penjualan(models.Model):
    ...
    detailpenjualan_ids = fields.One2many(
        comodel_name='techmart.detailpenjualan',
    ...
    def _compute_totalbayar(self):
        for line in self:
            result = sum(self.env['techmart.detailpenjualan'].search(
    ...
    def unlink(self):
        if self.detailpenjualan_ids:
            penjualan = []
            for rec in self:
                penjualan = self.env['techmart.detailpenjualan'].search([('penjualan_id', '=', rec.id)])
                print(penjualan)
    ...
    def write(self, vals):
        for line in self:
            data_asli = self.env['techmart.detailpenjualan'].search([('penjualan_id', '=', line.id)])
            print(data_asli)
            ...
            for line in self:
                data_setelah_edit = self.env['techmart.detailpenjualan'].search([('penjualan_id', '=', line.id)])
                print(data_asli)
                print(data_setelah_edit)

                for data_baru in data_setelah_edit:
                    if data_baru in data_asli:
                        print(str(data_baru.barang_id.name) + " " + str(data_baru.qty) + ' ' + str(data_baru.barang_id.stok))
...
class DetailPenjualan(models.Model):
    _name = 'techmart.detailpenjualan'
    _description = 'Detail'

    name = fields.Char(string='Nama')
    penjualan_id = fields.Many2one(
        comodel_name='techmart.penjualan',
        string='Detail Penjualan')
    barang_id = fields.Many2one(
        comodel_name='techmart.barang',
        string='List Barang')
    ...
    def create(self, vals):
        ...
        if line.qty:
            self.env['techmart.barang'].search(
    ...
    def check_quantity(self):
        for line in self:
            if line.qty < 1:
                raise ValidationError('Jumlah pembelian minimal 1 !!')
